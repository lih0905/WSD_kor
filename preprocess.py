...
def process(sent_input, multiwords_set):
    ...
    try:
        for word_d in sent_input['morpheme']:
            ...
            if word_d['form'] in multiwords_set:
                for wsd_d in sent_input['WSD']:
                    ...
                    if word_d['form'] == wsd_d['word'] and word_d['word_id'] == wsd_d['word_id'] and word_d['label'] == wsd_d['pos']:
                        ...
                        break
                else: # 우리말샘 기준 다의어이긴 하나 WSD 데이터에 없는 경우는 제외
...
if __name__ == "__main__":
    ...
    with open(os.path.join(dict_path, dict_fname), 'rb') as f:
            urimal_dict = pickle.load(f)
    multiwords_set = set(k for k, v in urimal_dict.items() if len(v['definition'])>1)
    print(f"우리말샘 사전 기준 다의어는 {len(multiwords_set)}개입니다.")
    ...
    if args.data:
        ...
        for paragraph in data:
            for sentence in paragraph['sentence']:
                ...
                # 다의어가 하나 이상인 문장만 남김
                if len([w for w in sent if w[-1] != -1]) > 0:
                    i += 1
                    if i % 10 != 0:
